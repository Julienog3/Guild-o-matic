  ...
  return <form className="flex flex-col gap-8 h-full" onSubmit={(e: FormEvent) => onSubmit(e, credentials)}>
    ...
    <div className="flex flex-col gap-4 h-full">
      <div className="flex flex-col">
        <label className="text-light-gray mb-2 text-sm" htmlFor="password">E-mail</label>
        <input 
      ...
      <div className="flex flex-col">
        ...
        />
        <Link className="text-accent-blue self-end text-sm" to="/">Mot de passe oublié ?</Link>
      </div>
    </div>
    <div className="flex flex-col justify-center gap-6">
      <button type="submit" className="bg-accent-blue rounded-lg p-4 text-sm text-white">Se connecter</button>
      <p className="text-white text-center text-sm">Vous n&apos;avez pas de compte ? <Link className="text-accent-blue" to="/">S&apos;inscrire</Link></p>
    </div>
  </form>
}
